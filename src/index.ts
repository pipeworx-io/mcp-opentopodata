...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Open Topo Data');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'datasets': {
      // OpenTopoData has two datasets routes: /v1/datasets.json requires a
      // ?locations= param (echoes back which dataset served a query),
      // while the unversioned /datasets is the parameter-free index of
      // available datasets. We want the latter — bypass the /v1 BASE.
      const res = await pwFetch('https://api.opentopodata.org/datasets', {
        headers: { Accept: 'application/json', 'User-Agent': UA },
      });
      if (!res.ok) throw await httpError(res, 'Open Topo Data datasets');
      return res.json();
    }
    default:
...
async function topoGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'Open Topo Data');
  return res.json();
}
